// =============================================================================
// RRM Academy Design System — Figma Plugin Script
// =============================================================================
// HOW TO RUN:
//   1. Open your Figma file
//   2. Go to Plugins > Development > New Plugin
//   3. Choose "Figma Design" + "Run once"
//   4. Replace the contents of code.ts with this entire file
//   5. Click Run (play button)
//   6. The design system page will be created on your current page
//
// ALTERNATIVE: Install "Scripter" from Figma Community, open it,
...
function solidFill(h: string): Paint[] {
...
async function main() {
  ...
  try {
    ...
    // Font not available in Figma — try common alternatives
    try {
      await figma.loadFontAsync({ family: 'Cormorant Garamond SemiBold', style: 'Regular' });
      cormorantLoaded = true;
    } catch {
      console.log('Cormorant Garamond not loaded — using Inter Bold as fallback for headings. Add Cormorant Garamond to your Figma fonts to fix.');
    }
  }
  ...
  cursorY = createNote(page, 'Fonts: Cormorant Garamond SemiBold (headings) + Inter Regular/Medium (body). Both free Google Fonts.', cursorY);
  cursorY += 16;
  ...
  btnX = createButton(page, 'Disabled', 'disabled', btnX, btnY);
  cursorY += 56;
  ...
  hoverX = createButton(page, 'Hover  #4c3e54', 'primary-hover', hoverX, hoverY);
  cursorY += 56;
  ...
  sizeX = createButton(page, 'Large', 'large', sizeX, sizeY);
  cursorY += 64;
...
function createSectionHeading(page: PageNode, text: string, y: number): number {
...
function createGroupLabel(page: PageNode, text: string, y: number): number {
  const label = figma.createText();
...
function createSwatchRow(page: PageNode, swatches: any[], y: number): number {
  let x = 0;
...
function createTypeSpecimen(page: PageNode, style: any, y: number, cormorantLoaded: boolean): number {
...
function createButton(page: PageNode, label: string, variant: string, x: number, y: number): number {
...
function createSpacingBar(page: PageNode, name: string, px: number, y: number): number {
  // Label
  const label = figma.createText();
  label.fontName = { family: 'Inter', style: 'Regular' };
  label.fontSize = 11;
...
function createRadiusDemo(page: PageNode, name: string, px: number, x: number, y: number) {
...
function createShadowDemo(page: PageNode, name: string, effects: Effect[], x: number, y: number) {
  const box = figma.createRectangle();
  box.resize(140, 90);
  box.x = x;
  box.y = y;
...
function createNote(page: PageNode, text: string, y: number): number {
  // Background bar
