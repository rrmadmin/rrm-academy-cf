...
5. ~~**`/health` declared in openapi.json but no handler exists**~~ DONE (2026-07-05, PR #58) — `functions/health.js` returns text/plain `ok` per the openapi HealthStatus contract; `_middleware.js` excludes it from the trailing-slash 301. STILL PENDING: add `/health` to rrm-router `ASTRO_ROUTES` (separate repo) or apex traffic proxies to Wix.
6. ~~**Test glob silently skips files**~~ DONE (2026-07-05, PR #58) — 46 dark tests across 6 files now run (688 total). `tests/unit/supporter-gift.test.mjs` recovered 2026-07-05: the 2 stale assertions now assert the intentional 0-sentinel fail-soft (commit 63217f9d; consumers filter `gift_seq >= 1`) and the file is back in the test script. Still excluded: `tests/unit/fund-thermo.test.mts` (TS; CI Node 20 lacks type stripping). Playwright CI job for e2e still open.
7. ~~**`blog/posts.js:54` `SELECT * FROM posts`** with full bodies on hot path.~~ DONE (2026-07-05) — explicit `POST_COLUMNS` projection (drops unused `status`/`created_at`; future wide columns must opt in) + opt-in `limit`(1-200)/`offset` params; default no-param behavior byte-identical so `fetch-blog-data.mjs` unchanged. NOTE: `faqs.js` + `glossary/terms.js` carry the same latent `SELECT *` pattern — fix-one-grep-all candidates when next touched.
8. ~~**`partners/apply.js`** uses `Math.random()` for partner ID, no rate limit, no `withIdempotency`.~~ DONE (2026-07-05, PR #58) — crypto.randomUUID() (also in `src/lib/partners.ts` duplicate), 5/15min IP rate limit, withIdempotency (a retry previously created a duplicate row under a `-2`-suffixed slug).
9. ~~**`survey/request.js:105,109`** `env.DB` not guarded for the `sendEmail` log path.~~ DONE (2026-07-05, PR #58) — conditional log binding + `.catch()` on the failure logger.
10. ~~**N+1-ish correlated subqueries** in `community/posts.js` feed queries.~~ DONE (2026-07-05, PR #59) — LEFT JOIN grouped derived tables in both feed queries + `comments.js` tier-label probe; verified old-vs-new equivalence against seeded node:sqlite. Single-post paths intentionally keep the correlated form (optimal at N=1).
11. ~~**`_webhook-checkout.js`** has ~17 sequential `.run()/.first()` outside `db.batch([…])`.~~ WON'T-FIX / answered-by-review (2026-07-05) — full write-map review found NO contiguous D1-write pair batchable without changing semantics: every write is separated by a `meta.changes` branch, a mutually-exclusive path, or a Stripe/SES boundary. Batching the auto-create+welcome-token pair would introduce dangling tokens on the race path and invert the deliberate fail-soft. Partial-state risk is covered by the dispatcher 5xx-rollback + Stripe retry + idempotent statements; the true multi-table unit (enrollment+includes) is already batched inside `enrollUser`. PG4's warn on this file is a reviewed false positive — leave it warning. SEPARATE REAL ITEM: the L553 3DS race on the email-match fallback (`stripe_active` flip + "cancel Wix" admin email without checking sub status). Fix requires mirroring the metadata path's sub-status guard PLUS an email-match variant of the Sweep-2 reconciler in `_webhook-subscription.js` (current sweep keys on `sub.metadata.wix_subscription_id`, absent for cold checkouts). Low practical risk (Phase 3.1/3.2 route migrations through the metadata path); design task, not a patch.
...
15. ~~**`security.yml` triple-fires** on the typical claude/* → merge → main flow.~~ DONE (2026-07-05) — `claude/**` push trigger dropped; `main` push (full-history scan) + `pull_request` retained; pre-commit gitleaks covers local.
...
20. ~~**`courses/comments.js`** has no PATCH/DELETE handler.~~ DONE (2026-07-05) — verified gap not intent (original commit 467a9fd3 had no immutability statement; `updated_at` column existed unused). PATCH (author-only, POST-identical validation) + DELETE (author or admin+; explicit reply cleanup via `db.batch()` — one level suffices since POST only allows replies to top-level). Edit/Delete UI wired in the lesson player for own comments.
21. ~~**`courses/enroll.js:41`** no `withIdempotency` wrapper; double-tap can create two Stripe Checkout sessions.~~ DONE (2026-07-05) — layered: `withIdempotency` (client per-page-load key from the enroll buttons) replays completed responses with the same checkoutUrl; KV lock `enroll-stripe:${userId}:${courseId}` (1/10s, paid branch only) closes the concurrent window withIdempotency can't cover; frontend disables ALL enroll buttons during flight. Also closes the R2 review-gate gap (billed Stripe call previously had no rate limit). Free-enroll path untouched.
22. ~~**`merge.yml` duplicates 3 guards** that re-run in deploy.yml minutes later.~~ WON'T-FIX (2026-07-05) — the "duplication" is intentional pre-merge protection: merge.yml gates run BEFORE `git push origin main` (and the M1 invariant mints the App token only after the gates); deploy.yml's copies run post-merge and can only stop deployment, not a poisoned main. The ~30-60s per merge is the price of merge-blocking. Original item overstated.
23. ~~**AI Search reconcile weekly cron** refetches 4 data sources with no `actions/cache` step.~~ OBSOLETE (2026-07-05) — the weekly cron was retired 2026-05-27 (schedule commented out; manual dispatch kept for rollback). Do NOT add a cache if revived: the reconcile is destructive (`--reconcile --execute` deletes orphans) and cached day-old data would classify freshly published records as orphans and delete them from the index.
...
30. ~~**`/api/community/upload` returns `{ url }` bare**, not the `{ ok: true, ... }` convention used by sibling endpoints.~~ DONE (2026-07-05) — success response now `{ ok: true, url }`; sole caller (`src/pages/community/index.astro` `wireImageUpload`) reads `.error`/`.url` only, unaffected.
