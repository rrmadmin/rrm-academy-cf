...
6. ~~**Test glob silently skips files**~~ DONE (2026-07-05, PR #58) — 46 dark tests across 6 files now run (688 total). Still excluded, with reasons in the test script comment: `tests/unit/supporter-gift.test.mjs` (2 assertions predate the intentional 0-sentinel fail-soft in `_supporter-gift.js` — needs payment-surface judgment, 10 passing tests recoverable) and `tests/unit/fund-thermo.test.mts` (TS; CI Node 20 lacks type stripping). Playwright CI job for e2e still open.
