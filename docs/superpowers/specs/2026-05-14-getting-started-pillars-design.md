...
- Per-phase deploy-infrastructure gates: edit `rrm-router/src/index.js` `ASTRO_ROUTES`, register pillar in `ssot/guides.json`, register pillar in `/guides/` catalogue source, bump `deploy.yml` `guides_count` assertion (current `-ne 11` → 12 → 13), run CI gate `scripts/gates/validate-guides-registry.mjs`
...
2. **`ssot/guides.json` gate**, pillar registered with all required fields; CI gate passes.
3. **`/guides/` catalogue gate**, pillar source-of-truth entry; build-time rendering verified.
4. **`deploy.yml` `guides_count` bump**, the hardcoded `if [ "$guides_count" -ne 11 ]` is bumped per phase: 11→12 (Phase 1 PR), 12→13 (Phase 2 PR). Alternative: refactor to read length from `ssot/guides.json` once, eliminating the hardcoded constant.
...
2. `ssot/guides.json` includes the new pillar entry; CI gate `scripts/gates/validate-guides-registry.mjs` passes.
3. `/guides/` catalogue source includes the new pillar; `/guides/` page renders the new pillar card.
4. `deploy.yml` `guides_count` bumped per phase: 11→12 (Phase 1), 12→13 (Phase 2). Alternative refactor reads length from `ssot/guides.json` in Phase 1 PR.
...
2. Remove the new pillar's `ssot/guides.json` entry; CI propagates removal to 6 downstream consumers.
...
| D19 | Per-phase deploy-infrastructure gates: rrm-router ASTRO_ROUTES + ssot/guides.json + /guides/ catalogue + deploy.yml count bump + OG image render + app-shell wrap | Prevents 404 / wrong-content launches |
...
| D33 | `deploy.yml` `guides_count -ne N` assertion bumped per phase (11→12→13); alternative refactor reads length from `ssot/guides.json` in Phase 1 PR | Hardcoded count constant traps every phase otherwise |
