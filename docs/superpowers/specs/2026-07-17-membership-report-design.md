...
- **Membership definitions:**
  ...
  - `headline.total_supporters` = distinct lowercase emails across (paying STUC roster) UNION (non-refunded `donor_gift` givers in the reporting month) UNION (course buyers in the reporting month) -- one human counts once. `delta_vs_prior_month_cents` compares `recurring_monthly_cents` only. Both definitions are part of the G1 contract and the G5 hand check.
