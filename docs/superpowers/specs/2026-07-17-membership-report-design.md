...
- **Membership definitions:**
  ...
  - `headline.total_supporters` = distinct lowercase emails across (paying STUC roster) UNION (non-refunded `donor_gift` givers in the reporting month) UNION (course buyers in the reporting month) -- one human counts once.
  - **Month-over-month (r4, Brian 2026-07-17, supersedes the mixed-basis delta):** the Change tile/section compares LIKE FOR LIKE only. `headline.mom` = `{ receipts_this_month_cents, receipts_prior_month_cents, supporters_this_month, supporters_prior_month, month_in_progress }` where receipts = non-refunded `donor_gift` membership receipts per calendar month (same series the trend chart uses) and supporters = the distinct-email supporter definition evaluated per month (givers/buyers in that month plus the roster leg for the current month only, prior months from realized activity). `month_in_progress=true` when the reporting month is the current ET month; renderers must then phrase it as "so far" (e.g. "July so far: $397 vs June: $568"). **Date-aware anticipation (r4b):** when in progress, `mom.receipts_anticipated_cents` = collected-so-far + monthly amounts of active paying subs whose next scheduled renewal (Wix COALESCE(next_expected_at, last_order_at+31d); Stripe current_period_end) falls in [now, monthEnd); watchlist (voided/past_due) and KNOWN_PAUSED excluded from anticipation. Rendered as an estimate with the word "about" mandatory ("$X collected, about $Y expected by month end"). Absent/null for completed months. The old `delta_vs_prior_month_cents`/`delta_basis` roster-MRR-vs-receipts comparison is REMOVED from both renderers (field may remain in JSON for compatibility but nothing renders it). Roster MRR stays a point-in-time tile with no delta. Part of the G1 contract.
