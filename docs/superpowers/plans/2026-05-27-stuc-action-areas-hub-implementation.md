...
> **STATUS: NOT READY TO EXECUTE.** The design is not fully locked. **Phase 0 (Decisions Required) must be resolved with Brian + Naomi before any task below runs.** This plan is a prep artifact written 2026-05-27; do not start implementation without an explicit go-ahead. Several tasks (the seed especially) are hard-blocked on Phase 0.
...
**New files:**
- `migrations/024-stuc-action-areas.sql` — tables + `community_post.area_id` + CHECK enums + indexes (number per D0-6).
- `migrations/025-stuc-action-areas-seed.sql` (or run via admin endpoints) — seed areas + owner-resolution UPDATE. Blocked on D0-1/D0-2.
...
**Modified files:**
- `functions/_middleware.js` — carve `/community` (exact) + `/community/areas` + `/community/areas/*` out of `needsAuth` (both the ~line-186 and ~line-300 blocks); re-scope the guard invariant to `/community/events`, `/community/members`, `/community/post`; `npm run guard:update`. **Hash-guarded file.**
...
**Files:** `migrations/024-stuc-action-areas.sql` (create), `schema.sql` (verify/regenerate).
...
- [ ] **1.2 — Confirm migration number** (`ls migrations/ | tail -1`; expect `023`, use `024` unless D0-6 reassigned).
- [ ] **1.3 — Write `migrations/024-stuc-action-areas.sql`** with the exact DDL from the spec §Data model (verbatim — the 5 `CREATE TABLE IF NOT EXISTS`, the `ALTER TABLE community_post ADD COLUMN area_id`, and all 9 indexes). Key invariants baked into the DDL: `slug … UNIQUE COLLATE NOCASE`, every enum a `CHECK (… IN (…))`, FKs are decorative (D1 ignores them).
...
- [ ] **2.1 — Read the current `needsAuth` predicate** (the ~line-186 block and the ~line-300 block — line numbers drift; grep `needsAuth` and `/community`). Confirm both currently match `/community` exact + `/community/*`.
...
- [ ] **3.1 — `_shared.js`: `validateAreaId(env, id)`** — returns true only if the area exists AND `status='active'`. Reused by `posts.js` POST and all admin writes carrying an `area_id`.
...
- [ ] **5.3 — DELETE coordination:** read the current `posts.js` DELETE batch (the realtime-STUC spec edits the same handler for `stuc_meeting` cleanup). Produce ONE combined cleanup batch; do not append blindly.
...
**Files:** `migrations/025-stuc-action-areas-seed.sql` (or admin-endpoint seed).
