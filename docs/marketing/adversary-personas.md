# Adversary Personas: Anti-RRM Opposition

> These personas model the people and institutions actively working against RRM's credibility. Understanding their mental models, incentive structures, and rhetorical strategies is essential for positioning RRM Academy content that doesn't fight them head-on but makes their arguments irrelevant.

**Last updated:** 2026-03-01
**Sources:** ACOG/ASRM published position documents, media coverage (.firecrawl/), Dr. Naomi Whittaker public transcripts (5 podcasts/masterclasses, 2020-2025)

---

## Persona 1: Dr. Institutional REI

**Archetype:** Board-certified reproductive endocrinologist, 15-25 years in practice, affiliated with a major academic fertility center or large private IVF clinic chain.

### Demographics

| Attribute | Detail |
|-----------|--------|
| Title | MD, FACOG, board-certified REI |
| Practice | Academic medical center REI division or multi-location IVF franchise |
| Age | 42-60 |
| Income | $400K-$800K (procedural + ART cycle volume) |
| Professional orgs | ASRM fellow, ACOG fellow, possibly SRS member |
| Media presence | Quoted in media as "fertility expert," may have podcast/social following |

### Belief System

| Belief | Strength | Notes |
|--------|----------|-------|
| IVF is the gold standard for infertility treatment | Core identity | Career built on this. IVF is what they trained to do. |
| RRM rebrands basic gynecology | High conviction | "This is just OBGYN 101 with a marketing name" |
| RRM delays treatment, shrinking fertility windows | Genuine concern (mixed with self-interest) | Age-related decline is real. But used to pressure patients into ART sooner. |
| RRM is religiously motivated pseudoscience | Medium-high | NaProTechnology's Catholic origins make this easy to assert |
| "Unexplained infertility" is a legitimate diagnosis | Unquestioned assumption | RRM's position that "unexplained" = "undiagnosed" directly threatens this |
| Large-scale RCTs are the only valid evidence | Epistemological commitment | Used to dismiss retrospective cohort studies that show RRM works |
| IVF success rates validate the approach | Key metric | Selective use of per-transfer rates, not per-patient cumulative rates |
| Cervical mucus and progesterone are irrelevant to fertility workup | Trained blind spot | Dr. Naomi's IVF training professor literally laughed at her for asking about cervical mucus and progesterone. This is the culture. |
| Surgical skill for endo/adhesions is unnecessary when IVF exists | Institutional assumption | Near-contact laparoscopy study found 93.5% of women told they didn't have endometriosis actually did. The disease is missed because the bypassing approach makes diagnosis feel unnecessary. |

### The Training Gap

The REI's belief system is not malicious. It is structural. Dr. Naomi identifies "three shortcuts" in reproductive medicine history, each of which caused 10-20 years of surgical and diagnostic skill atrophy:

1. **Clomid (1950s-60s)** -- empiric ovulation induction. Doctors stopped investigating why ovulation wasn't occurring.
2. **The Pill (1960s)** -- suppressive management. Doctors stopped diagnosing the conditions the Pill masked.
3. **IVF (1970s-present)** -- bypassing conception entirely. Doctors stopped treating the reproductive system.

Each shortcut made the previous generation's diagnostic skills less necessary to learn, less practiced, and eventually lost. Today's REI fellowship is built on the third shortcut. The REI is not choosing to ignore diagnosis. The diagnostic tradition was never passed down to them.

### Incentive Structure

**Financial:**
- REI compensation runs $800K-$1.5M+. That salary exists because IVF/ART cycles generate $15K-$30K each at volume. The threat is not losing one cycle -- it's the entire compensation model that depends on ART being the default pathway.
- REI practices are increasingly private-equity-backed with volume targets and growth obligations.
- If RRM gains government recognition, insurance mandates, or public funding, that money comes from the same pool that currently funds IVF/ART. This is a zero-sum fight for policy dollars, not just patient dollars.
- Diagnostic workup (what RRM does) is low-margin compared to procedures. An REI who pivots to RRM-style diagnostics takes a massive pay cut.

**Professional:**
- Career advancement in academic REI requires publishing on ART outcomes and techniques.
- ASRM committee membership and speaking engagements reinforce ART-centric worldview.
- Colleagues and department chairs share the same training and assumptions.
- Publicly defending IVF against RRM wins social approval within the professional tribe.

**Psychological:**
- Sunk cost: 4 years medical school + 4 years residency + 3 years REI fellowship, all centered on ART.
- Cognitive dissonance: if RRM works for many patients, what does that say about the bypassing approach they've practiced for decades?
- Status quo bias: the current model is working (for the doctor, financially). Why change?
- Identity threat: The core narrative they've internalized and transmitted to patients is "Your body is broken. Nature is broken. IVF is better." RRM's existence says the body often isn't broken, it's mistreated or undiagnosed. That reframes the REI's career as the problem, not the solution.

### Rhetorical Playbook

| Tactic | Example | What it actually does |
|--------|---------|----------------------|
| Authority appeal | "ACOG and ASRM have both said..." | Substitutes institutional authority for engagement with evidence |
| Rebranding accusation | "RRM is just OBGYN care with a new name" | If true, there's no threat. If false, delegitimizes without refuting. |
| Clock anxiety | "Every month you delay IVF, your egg quality declines" | Real biological truth weaponized to discourage diagnostic workup |
| Political framing | "RRM is an anti-abortion tool designed to eliminate IVF" | Poisons the well. Audience stops evaluating evidence and starts evaluating politics. |
| Moving the goalposts | "Show me the RCTs" | Retrospective cohort studies with 1,310 patients get dismissed because they aren't randomized, which is ethically impossible for most fertility interventions |
| False binary | "You're either pro-science or pro-RRM" | Forces patients to choose tribe over treatment |
| Concern trolling | "I worry that RRM gives patients false hope" | Frames the critic as the compassionate one while dismissing evidence |
| Scare-tactic selling | "You need to freeze your eggs NOW or you'll regret it forever" | Dr. Naomi witnessed an IVF physician shadow appointment where the doctor used fear to close a $15K egg freeze on a young woman. The pressure is financial, not medical. |
| IUI as stalling | "Let's try a few rounds of IUI before IVF" | IUI was invented in the 1770s. Dr. Naomi calls it "not scientific." It's a revenue placeholder before the more lucrative IVF recommendation. |

### What They Fear

1. **Boyle 2025 and Sanchez-Mendez 2025** -- peer-reviewed outcomes studies that show comparable or superior results to IVF in the same populations they treat. These studies cannot be dismissed as "anecdotal."
2. **The cost argument** -- a $2K-$5K RRM workup vs. $40K-$60K multi-cycle IVF is devastating in a cost-conscious healthcare market, especially as insurance mandates grow.
3. **The obstetric safety data** -- lower prematurity, lower multiple pregnancy rates, lower low birth weight. These are outcomes ACOG itself cares about.
4. **Patient self-education** -- informed patients who ask "Why was I told my infertility was unexplained when you never tested my progesterone in the luteal phase?" undermine the authority relationship.
5. **Legislative momentum** -- Arkansas RRM insurance mandate (2025). If more states follow, government funding and insurance reimbursement flows toward RRM and away from ART. This is the existential threat: not losing patients one at a time, but losing the policy infrastructure that sustains $1M+ REI salaries.
6. **The missed-disease data** -- near-contact laparoscopy study showing 93.5% of women told "you don't have endo" actually did. This isn't an evidence gap. It's a malpractice exposure.
7. **IVF success rate stagnation** -- per-cycle live birth rate has hovered around 28% since the technology was introduced, despite marketing that implies continuous improvement. When patients learn this, the "gold standard" framing collapses.

### How to Make This Persona Irrelevant (Not How to Fight Them)

- **Don't debate. Publish.** Every citation on rrmacademy.org makes the "no evidence" claim harder to sustain.
- **Let patients discover the comparison.** The Boyle 2025 charts (41% RRM vs. 30.6% single-embryo IVF) speak without polemics.
- **Address clock anxiety genuinely.** RRM workup takes 2-3 cycles (4-6 months). IVF prep takes 2-3 months. The delay differential is smaller than claimed. State this with data, not defensiveness.
- **Never mention ACOG/ASRM by name on the patient site.** Don't amplify their audience. Respond to the ideas, not the institutions.

---

## Persona 2: The Institutional Gatekeeper (ACOG/ASRM)

**Archetype:** Not a single person but a committee-driven institutional voice. These organizations issue position statements, fact sheets, and media responses that become the authoritative reference for journalists, legislators, and search engines.

### Organizational Profile

| Attribute | ACOG | ASRM |
|-----------|------|------|
| Members | 65,000+ OBGYNs | 8,000+ REI specialists |
| Revenue model | Membership dues, CME, advocacy | Membership dues, journal publishing, conference |
| Governance | Elected officers, practice advisory committees | Board + special interest groups (SRS, SART) |
| Political stance | Progressive on reproductive rights | Progressive on reproductive rights |
| RRM position | Formal issue brief opposing RRM | Dedicated opposition hub with multiple documents |

### Their Core Arguments (Verbatim)

**ACOG issue brief (2025):**
- "RRM is not a medical term."
- "RRM can expose patients to needless, painful surgical interventions."
- "The RRM movement is, at its roots, tied to the so-called personhood effort."
- "ACOG opposes policies that seek to divert patients away from the full range of evidence-based assisted reproductive technologies toward the unproven concept of RRM."

**ASRM fact sheet (2025):**
- "Restorative Reproductive Medicine overlooks IVF, male-factor care, and the need for full-spectrum fertility treatment."
- "Don't be fooled: There is no substitute for IVF."
- "'Restorative Reproductive Medicine' and 'Ethical IVF' are Misleading Terms That Threaten Access."

### Why They Oppose RRM

**Stated reasons:**
- Patient safety (delay of treatment)
- Scientific rigor (insufficient RCTs)
- Reproductive autonomy (concern that RRM replaces patient choice with ideology)

**Unstated but structural:**
- **Revenue protection:** ASRM's membership is REI specialists whose income depends on ART volume. If RRM captures even 10% of the IVF market, that's billions in lost revenue industry-wide.
- **ICD-10 coding capture:** WHO reclassified infertility as a disease (not a symptom). This enables IVF to be coded and billed as treatment for a disease, positioning ART as the default insured treatment pathway. Dr. Naomi identifies this as a deliberate strategy to funnel government funding and insurance dollars exclusively to IVF/ART. If infertility is a disease, and IVF is the treatment, then diagnostic workup (RRM) is just the preamble before the "real" treatment.
- **Jurisdictional defense:** "RRM is not a medical term" is a turf claim. If RRM becomes a recognized specialty, it competes for patients, training slots, and insurance reimbursement.
- **Political positioning:** Both organizations have invested heavily in framing reproductive technologies as reproductive rights. RRM complicates this narrative because it offers reproductive options that don't involve embryo creation.
- **Search dominance:** ASRM's "Just the Facts" fact sheet ranks #1 for "RRM vs IVF." They benefit from controlling the narrative in search.

### How They Operate

1. **Issue briefs and fact sheets** -- published on their websites, designed to be the top Google result for RRM queries. Written by committee, tone of institutional authority.
2. **Media seeding** -- when journalists write about RRM (usually triggered by legislation), ACOG/ASRM press offices provide ready-made quotes and expert contacts. This is why every major RRM article includes an ASRM/ACOG rebuttal.
3. **Letter campaigns** -- ASRM organized letter-writing against IIRRM. Grassroots opposition that appears organic but is institutionally coordinated.
4. **Terminology warfare** -- refusing to use the term "RRM" (calling it "so-called RRM" or putting it in scare quotes) denies linguistic legitimacy.
5. **Political framing** -- linking RRM to anti-abortion and personhood politics makes the medical conversation impossible. Once RRM is coded as political, journalists treat it as a controversy rather than a clinical approach.

### Vulnerabilities

1. **Their own guidelines contradict their anti-RRM stance.** ACOG recommends hormonal evaluation, cycle tracking, and surgical treatment of endometriosis -- all things RRM does. The objection is to the name and the paradigm, not the practices.
2. **Selective evidence standards.** IVF was never validated with RCTs against expectant management before becoming standard of care. Demanding RCTs from RRM that IVF never had is a double standard.
3. **Obstetric outcomes.** ACOG's own maternal health priorities (reducing prematurity, reducing multiples) align with RRM's superior safety data. This creates internal tension.
4. **Patient autonomy argument cuts both ways.** If ACOG supports patient choice, opposing insurance coverage for RRM restricts that choice.
5. **The "just basic gynecology" claim is self-defeating.** If RRM practices are basic gynecology, then they should be covered by insurance and recommended by ACOG. You can't simultaneously say "this is nothing new" and "this is dangerous."

---

## Persona 3: The Skeptical Academic

**Archetype:** Evidence-based medicine researcher, epidemiologist, or clinical methodologist who is not hostile to RRM's concept but dismisses the evidence as methodologically insufficient.

### Profile

| Attribute | Detail |
|-----------|--------|
| Role | Associate professor, department of reproductive epidemiology or clinical research |
| Training | MD-PhD or PhD in epidemiology/biostatistics |
| Orientation | Genuinely committed to evidence hierarchy |
| Stance on RRM | "Interesting but unproven" |

### Their Argument

"I have no problem with the idea of treating root causes. But the evidence base is retrospective cohort studies with no control groups. Sanchez-Mendez 2025 is the largest, and it's impressive, but it's still not randomized. Selection bias, referral bias, and healthy user effects could explain the results. Show me a well-designed RCT and I'll change my position."

### Where They're Right

- The evidence hierarchy does place RCTs above retrospective cohorts.
- Selection bias is a legitimate concern (couples who seek RRM may be more motivated).
- Some RRM outcome studies have small sample sizes or single-center design.

### Where They're Wrong (Or Unfair)

- **Ethical impossibility of RCTs:** You cannot randomize couples to "no treatment" or "IVF only" when they're seeking RRM. IRBs won't approve it. This isn't unique to RRM -- most surgical interventions lack RCTs.
- **Double standard:** IVF was adopted as standard of care based on case series and uncontrolled studies. ICSI was never subjected to an RCT before worldwide adoption.
- **Dismissal of Kaplan-Meier analysis:** Sanchez-Mendez 2025 uses proper survival analysis to adjust for censoring. This is not a crude observational count.
- **Ignoring dose-response:** Multiple independent studies across different countries, clinics, and timeframes showing consistent results (29-66% adjusted take-home baby rates) is convergent evidence.

### How to Win Them Over

This persona is the only adversary who can be persuaded with evidence. They're not financially or politically motivated.

- **Cite the methodology, not just the results.** "Kaplan-Meier with log-rank tests for subgroup comparison" signals methodological rigor.
- **Acknowledge limitations explicitly.** The pillar page already does this ("No large-scale randomized controlled trials exist yet").
- **Frame the evidence correctly.** "Level 2b evidence from multiple independent cohort studies" is accurate and respectable.
- **Point to the comparative evidence gap for IVF.** IVF's evidence base for per-patient cumulative outcomes is surprisingly thin compared to per-cycle/per-transfer rates.

---

## Persona 4: The Media Framer

**Archetype:** Health/policy journalist at a major outlet (NYT, STAT News, The 19th, Washington Post) who covers reproductive health through a reproductive-rights lens.

### Profile

| Attribute | Detail |
|-----------|--------|
| Beat | Reproductive health, policy, women's health |
| Political orientation | Progressive |
| Source network | ACOG/ASRM press offices on speed dial |
| Knowledge of RRM | Surface-level, learned about it when covering legislation |
| Primary frame | "Anti-abortion movement targets IVF" |

### How They Frame RRM

The dominant media frame (from actual coverage):
- RRM = part of the anti-abortion/personhood movement
- RRM clinics = "alternatives pushed by anti-IVF activists"
- Patient motivation = religious conviction (primary), "crunchy" lifestyle (secondary), cost (tertiary)
- The word "obscure" appears in almost every article
- MAHA (Make America Healthy Again) connection amplified to code RRM as politically right-wing

### What They Get Wrong

1. **Religion as primary motivation.** The Sanchez-Mendez 2025 cohort is from Madrid. The Boyle studies are from Dublin. These are not American evangelical patients. The patient base is medically motivated.
2. **"Obscure" framing.** A field with 3,000+ peer-reviewed articles in a curated research library, published in Frontiers journals, with fellowship-trained physicians, is not "obscure." It's under-recognized.
3. **False equivalence.** Placing one quote from an RRM physician against three quotes from ACOG/ASRM creates an asymmetric "both sides" that inherently delegitimizes RRM.
4. **Conflation of NaProTechnology with all RRM.** NaPro is one approach within RRM. Covering NaPro's Catholic origins as if they define the entire field is reductive.

### Why They Frame It This Way

- **Source access:** ACOG/ASRM press offices are staffed and responsive. IIRRM and RRM Academy are small organizations without dedicated communications teams.
- **Narrative convenience:** "Anti-abortion movement targets IVF" is a clean, compelling story. "Doctors disagree about diagnostic approach to infertility" is boring.
- **Audience expectations:** Readers of The 19th and STAT expect coverage through a reproductive rights lens. RRM doesn't fit neatly into either side.
- **Risk aversion:** Writing sympathetically about RRM risks backlash from the reproductive rights community and ACOG/ASRM relationships.

### Unreported Stories They're Missing

1. **Egg freezing influencer campaigns.** Dr. Naomi identifies that egg freezing marketing to young women is often undisclosed paid advertising. Influencers promote egg freezing as empowerment without disclosing financial relationships with IVF clinics. This is a consumer protection story that progressive outlets would cover aggressively if they knew about it.
2. **Organ removal culture.** Women having organs removed during surgery without prior consent or knowledge. Dr. Naomi: "We still have that hysteria mindset." Patients waking up from surgery to discover a fallopian tube or ovary was removed. This is a bodily autonomy story.
3. **The veterinary comparison.** Bovine reproductive medicine has better literature on chronic endometritis than human medicine. Dr. Naomi: "The cows are worth more than us." Agricultural economics drove research investment that women's health never received.
4. **Suppressive medicine as gaslighting.** The standard of care for endometriosis, PCOS, and painful periods is hormonal suppression that masks symptoms while disease progresses. Women are told they're "treated" while getting worse.

### What Would Change the Coverage

- **A communication-ready organization.** Journalists use whoever responds fastest with clear quotes. RRM Academy having a media kit, press page, and responsive contact changes the source balance.
- **Patient stories that break the frame.** A secular, politically progressive couple who chose RRM over IVF for medical reasons (not religious ones) undermines the "anti-abortion tool" narrative.
- **The cost angle.** Economic inequality and IVF access is a story progressive outlets want to tell. RRM as affordable alternative to $40K+ IVF cycles is the frame that works with their audience, not against it.
- **International evidence.** Madrid. Dublin. Australia. The global dimension makes the "American culture war" frame harder to sustain.
- **The egg freezing investigation angle.** Undisclosed paid influencer partnerships with IVF clinics pushing egg freezing on young women is a media-ready scandal. This reframes RRM not as anti-IVF but as pro-informed-consent.
- **The missed-disease angle.** 93.5% of women told they didn't have endometriosis actually did. This is a medical scandal, not a religious debate.

---

## Cross-Persona Patterns

### Shared Tactics

| Tactic | Used by | Mental model |
|--------|---------|-------------|
| Political poisoning | All four | Confirmation bias -- once coded as "anti-abortion," all evidence is filtered through that lens |
| Terminology denial | Institutional, REI | Authority bias -- "if ACOG doesn't recognize the term, it isn't real" |
| Evidence threshold manipulation | REI, Academic | Moving the goalposts -- demand RCTs that are ethically impossible |
| Clock pressure | REI | Loss aversion + hyperbolic discounting -- "act now or lose your chance" |
| Rebranding accusation | REI, Institutional | Anchoring -- if it's "just basic OBGYN," it can't be worth separate attention |

### What All Adversaries Share

1. **None of them engage with the published outcomes data.** ACOG's issue brief cites zero RRM outcome studies. ASRM's fact sheets cite their own position papers, not Boyle or Sanchez-Mendez.
2. **The political frame is the primary weapon.** Without the abortion/personhood connection, the opposition would be a normal scientific disagreement. The political frame makes it existential.
3. **None of them acknowledge the cost difference.** $2K-$5K vs. $40K-$60K is never addressed in any adversary document.
4. **None of them address obstetric safety.** Lower prematurity, fewer multiples, lower LBW -- these are outcomes ACOG explicitly cares about but ignores in the RRM context.
5. **The core narrative is shared across all four.** Dr. Naomi articulates what they all implicitly transmit to patients: "Your body is broken. Nature is broken. IVF is better." This is not a medical conclusion. It is a business model dressed as clinical judgment.
6. **None of them account for the training gap.** The "three shortcuts" (Clomid, Pill, IVF) each eliminated a generation of diagnostic skill. Today's adversaries aren't defending bad medicine knowingly. They're defending the only medicine they were taught.

---

## Strategic Implications for RRM Academy Content

### Do

- **Lead with evidence, not argument.** The charts and citations on what-is-rrm are the right approach. Data makes the adversary's silence conspicuous.
- **Address the clock anxiety directly with data.** Average time to conception in Boyle 2025: 12 months. Average IVF timeline (consultation to transfer): 3-6 months per cycle, 2-3 cycles. The real comparison is 12 months vs. 9-18 months + $40K-$60K.
- **Use the adversary's own standards against them.** "ACOG recommends evaluation and treatment of the underlying cause of abnormal uterine bleeding" (ACOG Practice Bulletin 128). RRM does exactly this.
- **Feature international evidence prominently.** Madrid, Dublin, Ireland, Australia. Undercuts the American culture war frame.
- **Never name ACOG/ASRM on patient-facing pages.** Don't amplify. Don't debate. Let the evidence stand.
- **Use the "three shortcuts" framework.** Clomid, the Pill, and IVF each eliminated a generation of diagnostic skill. This explains why the opposition exists without demonizing individual doctors. It's a systemic problem, not a moral one.
- **Name the core narrative to disarm it.** "Your body is broken. Nature is broken. IVF is better." Once a patient can articulate the narrative they've been sold, they can evaluate it critically.
- **Lead with the missed-disease data.** 93.5% of women told "you don't have endometriosis" actually did. This single statistic does more work than any argument about paradigms.
- **Use the menstrual cycle as EKG framing.** Dr. Naomi: "The menstrual cycle is the EKG of women." Nobody would suppress an abnormal EKG with medication and call it treatment.

### Don't

- **Don't be defensive.** Defensive tone signals weakness. Confident, clinical tone signals authority.
- **Don't fight the political frame.** You will not win a political argument. Win the medical one.
- **Don't overstate evidence.** "RRM works for many patients" is defensible. "RRM is better than IVF" is a claim the evidence doesn't universally support.
- **Don't target Dr. Institutional REI personally.** Some REIs are genuinely curious about RRM and may become allies. The adversary is the system, not the individual.
- **Don't use "three shortcuts" on patient-facing content.** This framework is for internal strategy and provider education. Patients don't need to know about medical training history. They need to know RRM works.
