...
export async function onRequestGet(context) {
  ...
  try {
    ...
    if (!env.SEO_MONITOR_API_TOKEN) {
      return json({ ok: false, error: 'SEO service not configured' }, 503);
    }
    ...
    if (action === 'observatory') {
      if (!env.OBSERVATORY_API_TOKEN) {
        return json({ ok: false, error: 'Observatory service not configured' }, 503);
      }
      ...
      try {
        ...
        return json({ ok: false, error: 'Observatory service unavailable' }, 502);
      }
    }
    ...
    switch (action) {
      ...
      case 'google-auth': {
        ...
        try {
          ...
          return json({ ok: false, error: 'OAuth consent failed' }, 502);
        }
      }
      ...
      case 'google-callback': {
        ...
        try {
          ...
          return json({ ok: false, error: 'OAuth callback failed' }, 502);
        }
      }
      ...
      if (action === 'cached') {
        ...
        if (lastResults) {
          try {
            ...
            return json({ ok: false, error: 'Invalid cached results' }, 500);
          }
        }
        return json({ ok: false, error: 'No cached results available' }, 404);
      }

      return json(data, workerResp.status);
    } catch {
      return json({ ok: false, error: 'SEO service unavailable' }, 502);
    }
  } catch {
    return json({ ok: false, error: 'SEO service error' }, 502);
  }
}
