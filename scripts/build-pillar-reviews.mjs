...
export function extractLastReviewed(astroSource) {
  // Strip line comments and block-comment-on-a-line cases before matching,
  // so `// const lastReviewed = '...'` doesn't get picked up as a real value.
  const stripped = astroSource
    .split('\n')
    .filter((line) => !/^\s*(?:\/\/|\/\*|\*)/.test(line))
    .join('\n');
  const match = stripped.match(/const\s+lastReviewed\s*=\s*['"]([^'"]+)['"]/);
