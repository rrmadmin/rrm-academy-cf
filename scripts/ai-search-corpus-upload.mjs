...
// Pillar pages are loaded at runtime from src/data/guides.json (generated by
// scripts/build-guides-data.mjs from each pillar's Astro source). Falls back to
// a minimal placeholder set if the file is missing -- preserves prior behavior
// for environments where guides.json hasn't been regenerated.
const PLACEHOLDER_PILLARS = [
...
function loadPillars() {
  const guidesPath = join(REPO_ROOT, 'src/data/guides.json');
  if (!existsSync(guidesPath)) {
    log(`warn: src/data/guides.json missing; falling back to placeholder pillars (${PLACEHOLDER_PILLARS.length}). Run scripts/build-guides-data.mjs first.`);
    return PLACEHOLDER_PILLARS;
  }
  try {
    const guides = JSON.parse(readFileSync(guidesPath, 'utf8'));
    if (!Array.isArray(guides) || guides.length === 0) throw new Error('empty or non-array');
    return guides.map(g => ({
      slug: g.slug,
      title: g.title,
      // Rich body: title + description + section headings + FAQ keywords + body prose.
      // Pattern mirrors scripts/embed-library-ci.mjs Guide loop for index parity.
      body: [g.title, g.description, g.sectionHeadings && g.sectionHeadings.length ? 'Sections: ' + g.sectionHeadings.join(', ') : null, g.keywordText, g.bodyText].filter(Boolean).join('\n\n'),
    }));
  } catch (e) {
    log(`warn: failed to parse src/data/guides.json (${e.message}); falling back to placeholder pillars.`);
    return PLACEHOLDER_PILLARS;
  }
}
