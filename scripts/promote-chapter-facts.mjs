#!/usr/bin/env node
/**
 * promote-chapter-facts.mjs — Promote staging fact files to D1 via
 * rrm-library-worker /promote-facts endpoint.
 *
 * Reads:
 *   /tmp/chapter-facts/<slug>.json   (from extract-chapter-facts.mjs)
 *
 * Calls:
 *   POST {worker}/promote-facts  (admin Bearer auth)
 *
 * Usage:
 *   node scripts/promote-chapter-facts.mjs --all
 *   node scripts/promote-chapter-facts.mjs --chapter chapter-63-...
 *   node scripts/promote-chapter-facts.mjs --dry-run --all
 *   node scripts/promote-chapter-facts.mjs --batch 20 --all
 */
...
if (!flags.all && !flags.chapter) {
  console.error('Usage: --all | --chapter <slug>  [--batch N] [--dry-run]');
  process.exit(1);
}
...
let files;
if (flags.chapter) {
  const p = join(STAGING_DIR, `${flags.chapter}.json`);
  if (!existsSync(p)) {
...
const failures = [];
const FAIL_PATH = '/tmp/chapter-facts/_failures.json';
...
for (const f of files) {
  try {
    ...
    // Drop `_validation` metadata from each fact before sending; keep only
    // /promote-facts schema fields. Per-fact guard: skip malformed entries
    // so one bad fact does not nuke the other good facts in the same file.
    let dropped = 0;
    const clean = [];
...
byChapter.forEach((c) => console.log(`  ${c.slug}: ${c.count} facts`));
