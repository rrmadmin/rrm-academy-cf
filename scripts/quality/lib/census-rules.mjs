...
const RULES = [
  ...
  [/^scripts\/redteam\//, 'E2E-DRIVER', 'The targeted red-team harness (auth, money, PII writes, leaks, headers, cost). Test infrastructure that lives under scripts/ so the same case table can also be driven at the live deployment (--mode live). Line coverage is the wrong instrument for the same reason it is wrong for test/**: covering the harness proves the harness ran, not that the site refused anything. Its correctness instruments are the run itself (a FAIL is a finding, never a loosened expectation) and test/redteam.test.js, which executes all 150 cases on every PR and additionally pins the case table structurally -- unique ids, a declared family, an adjudication note behind every `known` marker, and a dispatch entry for every route targets.mjs attacks.'],
  ...
  [/^scripts\/(guard|guard-ios-zoom|arise-hotspot-guard|agent-discovery-check|check-canonical-lockdown|check-types|check-bridge-links|check-persona-enum-sync|check-no-moved-paths|check-glossary-link-classes|check-suggest-coverage|check-render-gate-ids|check-cta-map|generate-design-tokens|audit-design-tokens|verify-citations|verify-templates|verify-build-output|validate-satellite-manifest|emit-schemamap-fallback|check-kit)\.mjs$/, 'PRODUCT-CODE', 'Deploy-blocking guard/check: runs in CI or pre-commit and gates what ships.'],
