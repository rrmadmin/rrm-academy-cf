...
export const EXTRA_DDL = [
  ...
  {
    path: 'migrations/042-bulk-campaign-lease.sql',
    why: 'newsletter_send.campaign and newsletter_send.updated_at are the bulk mail rail\'s per-campaign send lease, written by functions/api/newsletter/send.js\'s bulk path so two concurrent --send runs of one campaign cannot both mail the same head of the cohort; they live in the ROOT migrations/ directory, which the test replay list does not read, and postdate the 2026-05-27 snapshot; added 2026-09-11 with the migration in the same change. The migration is applied to remote rrm-auth before the code that binds these columns deploys, so gates:schema-drift stays level with live; until that apply lands, SD2 (STALE-PRESENT) is the expected and intended signal. Order matters: this file ALTERs newsletter_send, which schema.sql creates, so it must stay after schema.sql in the composition (EXTRA_DDL is applied last).',
  },
  {
    path: 'migrations/043-bulk-lease-unique.sql',
