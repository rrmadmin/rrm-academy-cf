...
const SECTION_PATTERN = new RegExp(
  ...
  [
    ...
    'OPINION\\s+STATEMENT',
    'CASE\\s+REPORT[S]?',
    'PARTICIPANT[S]?\\s*/\\s*MATERIAL[S]?(?:[,]?\\s+SETTING(?:[,]?\\s+METHOD[S]?)?)?',
...
const COMPOUND_REPAIRS: Array<{ pattern: RegExp; replacement: string }> = [
  ...
  { pattern: /WIDER\s+IMPLICATIONS?\s+OF\s+THE\s*\n+\s*FINDINGS?\s*:/gi, replacement: 'WIDER IMPLICATIONS OF THE FINDINGS:' },
  { pattern: /WIDER\s*\n+\s*IMPLICATION[S]?\s*:/gi, replacement: 'WIDER IMPLICATIONS:' },
  { pattern: /LIMITATION[S]?\s*\n+\s*REASONS?\s+FOR\s+CAUTION\s*:/gi, replacement: 'LIMITATIONS REASONS FOR CAUTION:' },
  { pattern: /STUDY\s+FUNDING\s*\n+\s*(?:\/\s*)?COMPETING\s+INTERESTS?\s*:/gi, replacement: 'STUDY FUNDING/COMPETING INTERESTS:' },
  { pattern: /STUDY\s+DESIGN\s*[,]?\s*\n+\s*SIZE\s*[,]?\s*(?:\n+\s*)?DURATION\s*:/gi, replacement: 'STUDY DESIGN, SIZE, DURATION:' },
  { pattern: /PARTICIPANT[S]?\s*\/?\s*MATERIAL[S]?\s*[,]?\s*\n+\s*SETTING\s*[,]?\s*(?:\n+\s*)?METHOD[S]?\s*:/gi, replacement: 'PARTICIPANTS/MATERIALS, SETTING, METHODS:' },
