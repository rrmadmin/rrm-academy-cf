  ...
  try {
    ...
    for (const item of arr) {
      ...
      if (typeof rec.url === 'string' && rec.url) {
        ...
        if (!u) continue;
        const type = typeof rec.type === 'string' ? rec.type : (pageTypeFromUrl(u) || 'article');
        out.push({
          ...rec,
          url: u,
  ...
  if (Array.isArray(result.pages)) {
    ...
    for (const p of result.pages) {
      ...
      out.push({
        ...
        type: typeof p.type === 'string' ? p.type : (pageTypeFromUrl(u) || 'article'),
...
export function initSavedShell(): void {
  ...
  const type = url ? pageTypeFromUrl(url) : null;
