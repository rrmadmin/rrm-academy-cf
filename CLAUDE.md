...
- **Subscribe**: KV-backed `checkRateLimit` (10/15min per IP) from `auth/_shared.js`; ELV check via `verifyAndTagEmail` before insert. INSERT uses `ON CONFLICT(email) DO NOTHING` for idempotency against UNIQUE(email) races. `complained` / `bounced` statuses are REFUSED on re-subscribe path (CAN-SPAM + SES reputation protection); only `unsubscribed` falls through to re-activation. Email validation via `validateBody({email:{type:'email'}})` (blocks control chars incl. null byte).
