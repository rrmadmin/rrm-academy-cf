 ...
 * report about a surface that holds somebody's contact details, their
 * records or their money. Everything else is a WARN, printed on every run
 * and never hidden.
 *
 ...
 *   live       queries the three sources. Runs PR-time in each consumer's
 *              own gate, before the build, because a lockfile only changes
 *              on a commit, so the commit is the right cadence (Brian's
 *              rule: targeted gates, no nightly sweeps). A source that
 *              cannot be reached FAILS the run: a gate that passes on an
 *              outage is not a gate.
 ...
 *
 * THIS IS THE VENDORED COPY, AND IT IS THE ONLY COPY. It is estate-shared
 * code owned by console-kit (`kit/packages/redteam/deps.mjs`), synced into
 * `vendor/redteam/` and sha-locked in `kit.lock.json`. Edit it here and it
 * will be overwritten by the next sync, and `check:kit` will call the edit
 * drift before that: fix it in the kit and sync.
 *
 * The consumer keeps `scripts/redteam/deps.mjs` as a CLI shim over this
 * module, and keeps `scripts/redteam/deps-accepted.json`, which is that
 * repo's own adjudication ledger and is never shared.
 *
 * Usage:
...
/**
 * THE CONSUMER'S ROOT, FOUND RATHER THAN ASSUMED. This module is vendored at
 * `<repo>/vendor/redteam/`, and the paths it defaults to (the lockfile, the
 * acceptance ledger, the dated report) belong to the REPO, not to the vendor
 * directory. Walking up to the nearest `package.json` finds it wherever the
 * kit chooses to put the vendor tree, so a later change of vendor layout
 * cannot silently point the gate at an empty lockfile.
 */
function repoRootFrom(start) {
  ...
  throw new Error(`deps: no package.json above ${start}; cannot locate the repo root`);
}
...
export const DEFAULTS = {
  ...
  accepted: join(ROOT, 'scripts', 'redteam', 'deps-accepted.json'),
};
