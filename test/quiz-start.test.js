/**
 * functions/api/quiz/start.js -- the FABM quiz-start conversion hook.
 *
 * It mirrors functions/api/endo-quiz/start.js (2026-08-21): the FABM quiz
 * conversion previously only fired when a visitor finished all nine
 * questions and requested their email result, so an ad visitor who started
 * the quiz and dropped off partway through was never counted at all.
 *
 ...
 *     gclid and the FABM Quiz Start conversion action;
...
describe('POST /api/quiz/start', () => {
  ...
  async function post({ headers = {}, ip = randomIp() } = {}) {
    const res = await start.onRequestPost({
      request: mockRequest('POST', {
        url: 'https://rrmacademy.org/api/quiz/start',
  ...
  it('buckets every IP-less request together under the literal "unknown"', async () => {
    ...
    // honour it: a full `rl:quiz-start:unknown` bucket must be the one an
    ...
    const bare = () => start.onRequestPost({
      request: mockRequest('POST', { url: 'https://rrmacademy.org/api/quiz/start' }),
      env, waitUntil,
    });

    await kv.put('rl:quiz-start:unknown', JSON.stringify({ count: 10, start: Math.floor(Date.now() / 1000) }));
    assert.equal((await bare()).status, 429, 'a full "rl:quiz-start:unknown" bucket did not govern an IP-less request');
    assert.deepEqual([...new Set(keysTouched)], ['rl:quiz-start:unknown'],
  ...
  it('uploads one conversion carrying the visitor\'s gclid and the FABM Quiz Start action', async () => {
    ...
    assert.equal(upload.body.destinations[0].productDestinationId, '7671519548');
